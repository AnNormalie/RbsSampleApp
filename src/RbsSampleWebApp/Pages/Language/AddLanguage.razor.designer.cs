...
{
    public partial class AddLanguageComponent : ComponentBase
    {
        ...
        {
            language = new RbsSampleWebApp.Models.RbsSampleDb.Language(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RbsSampleWebApp.Models.RbsSampleDb.Language args)
        {
            try
            {
                var rbsSampleDbCreateLanguageResult = await RbsSampleDb.CreateLanguage(language);
                UriHelper.NavigateTo("languages");
            }
            catch (System.Exception rbsSampleDbCreateLanguageException)
            {
